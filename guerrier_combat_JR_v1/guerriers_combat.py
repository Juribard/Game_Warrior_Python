...
print("BUT: Gagner le combat en faisant en sorte que les points de vie du GUERRIER IA soit entre 0 et -2  pv ou inférieur à -15 pv tout en économisant ces capacités.")
print("CONSEIL:")
conseil = random.randrange(1,3,1)
if conseil == 1:
    print("• Les Coups spéciaux ne sont pas esquivables mais sont limités")
elif conseil == 2:
    print("• Vous et le GUERRIER IA gagnent +1 coup special tous les 5 tours (jusqu'aux tours 25)")
else:
...
while pv_user > 0 and pv_pc > 0:
   # tour joueur
    ...
    if choix == 1:
        print("Je donne un coup de poing")
        choix = random.randrange(1,3,1)
        if choix == 1:
            print("Le GUERRIER IA esquive la dernière attaque du GUERRIER JOUEUR")
        else:
            degat = random.randrange(1,9,1)
            pv_pc = pv_pc - degat
            print ("Le GUERRIER JOUEUR a infligé à GUERRIER IA ", degat, "pv de dégats; Il lui reste ", pv_pc, "pv")
    elif choix == 2: 
        print("Je donne un coup de pied")
        choix = random.randrange(1,3,1)
        if choix == 1:
            print("Le GUERRIER IA esquive la dernière attaque du GUERRIER JOUEUR")
        else:
            degat = random.randrange(5,15,1)
            pv_pc = pv_pc - degat
            print ("Le GUERRIER JOUEUR a infligé à GUERRIER IA ", degat, "pv de dégats; Il lui reste ", pv_pc, "pv")
    elif choix == 3: 
        print("Je donne un coup de boule")
        choix = random.randrange(1,3,1)
        if choix == 1:
            print("Le GUERRIER IA esquive la dernière attaque du GUERRIER JOUEUR")
        else:
            degat = random.randrange(10,30,1)
            pv_pc = pv_pc - degat
            print ("Le GUERRIER JOUEUR a infligé à GUERRIER IA ", degat, "pv de dégats; Il lui reste ", pv_pc, "pv")
    elif choix == 4: 
        if special_user <=0:
            ...
            if special_user > 0:
                ...
                print ("Le GUERRIER JOUEUR a infligé à GUERRIER IA 40 pv de dégats; Il lui reste ", pv_pc, "pv")
            else:
                special_user = 0
    elif choix == 5:
        if soin_user <= 0:
        ...
        print ("Tu passes ton tour; Il reste ", pv_pc, "pv au guerrier ia.")

    print ("-----------------")
    # tour ia
    choix = random.randrange(1,6,1)

    if choix == 1:
        print("Le GUERRIER IA tente de te donner un coup de poing")
        print("Tu as 1 chance sur 2 pour esquiver. Choisir '1' ou '2'")
        choix1User = int(input("choix = "))
        if random.randrange(1,2,1) == choix1User:
            print("Le GUERRIER JOUEUR esquive la dernière attaque du GUERRIER IA")
        else:
            degat = random.randrange(1,9,1)
            pv_user = pv_user - degat
            print ("Le GUERRIER IA a infligé à GUERRIER JOUEUR", degat , "pv de dégats; Il lui reste ", pv_user, "pv")
    elif choix == 2: 
        print("Le GUERRIER IA tente de te donner un coup de pied")
        print("Tu as 1 chance sur 2 pour esquiver. Choisir '1' ou '2'")
        choix2User = int(input("choix = "))
        if random.randrange(1,2,1) == choix2User:
            print("Le GUERRIER JOUEUR esquive la dernière attaque du GUERRIER IA")
        else:        
            degat = random.randrange(5,15,1)
            pv_user = pv_user - degat
        ...
        print("Le GUERRIER IA tente de te donner un coup de boule")
        print("Tu as 1 chance sur 2 pour esquiver. Choisir '1' ou '2'")
        choix3User = int(input("choix = "))
        if random.randrange(1,2,1) == choix3User:
            print("Le GUERRIER JOUEUR esquive la dernière attaque du GUERRIER IA")
        else:        
            degat = random.randrange(10,30,1)
            pv_user = pv_user - degat
        ...
        if special_pc <=0:
            ...
            if special_pc > 0:
                ...
                print ("Le GUERRIER IA a infligé à GUERRIER JOUEUR 40 pv de dégats; Il reste ", pv_user, "pv") 
            else:
                special_pc = 0
    elif choix == 5:
        if soin_pc <= 0:
        ...
        print ("Le guerrier ia est choqué par l'attaque du GUERRIER JOUEUR! Donc, il passe son tour; Il lui reste ", pv_pc, "pv")
    ...
    if (pv_user < -2 and pv_user >= -15 and special_user >= 1):
        ...
        if chance == 1:
            ...
            pv_pc = pv_pc - degat
            print("Une attaque desesperée! le GUERRIER JOUEUR inflige à GUERRIER IA -", degat)
    if (pv_pc < -2 and pv_pc >= -15 and special_pc >= 1):
        ...
        if choix == random.randrange(1,2,1):
            ...
            degat = random.randrange(10,20,1)
            pv_user = pv_user - degat
            print("Une attaque desesperée! le GUERRIER IA inflige à GUERRIER JOUEUR -", degat)
